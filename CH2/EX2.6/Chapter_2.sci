...
function [ct] = modified_encrypt_caesar_general(pt,key)
    low=ascii('A')
    high=ascii('Z')
    a = ascii('A')
    l = length(pt)
    ...
    for i =1:l
        if isletter(part(pt,i:i)) then
            if (ascii(part(pt,i:i))<=high) then
                ct(i) = a + modulo( ascii(part(pt,i:i))+key-a, 26 ) 
            else
                ct(i) = ascii('a') + modulo( ascii(part(pt,i:i))+key-ascii('a'), 26)
            end
        else   
            ct(i) = ascii( part(pt,i:i) )
        end
...
//////Addition made here
function [ct] = modified_encrypt(pt)
    ct = modified_encrypt_caesar_general(pt,3)
endfunction

function [pt] = modified_decrypt_caesar_general(ct,key)
    low=ascii('A')
    high=ascii('Z')
    a = ascii('A')
    key = 26-key
    ...
    for i =1:l
        if isletter(part(ct,i:i)) then
            if (ascii(part(ct,i:i))<=high) then
                pt(i) = a + modulo( ascii(part(ct,i:i))+key-a, 26 )
            else
                pt(i) = ascii('a') + modulo( ascii(part(ct,i:i))+key-ascii('a'), 26 ) 
            end  
        else
            pt(i) = ascii(part(ct,i:i));        
        end
...
function [pt] = modified_decrypt(ct)
    pt = modified_decrypt_caesar_general(ct,3)
endfunction

function [a] = getvali(x)
    if(x==52) then
        a=32
    else
        if(x<=25) then
...
function [ct] = bitxr(a,b)
    ct=""
    for i=1:length(a)
        ct = ct + ascii(bitxor(ascii(part(a,ii)),ascii(part(b,i:i))))
    end    
end
